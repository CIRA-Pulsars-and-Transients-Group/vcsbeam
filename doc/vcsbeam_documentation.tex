...
The solutions are factored in the following way\footnote{It is as yet unclear if this gives a better result than ${\bf D} = ({\bf D}_d {\bf D}_b {\bf A}^{-1})^*$.},
\begin{equation}
    {\bf D} = ({\bf D}_d {\bf D}_b)^* {\bf A}^{-1},
\end{equation}
where
\begin{description}
