...
\begin{enumerate}
    \item ${\bf D}_d$ is in the basis $(p,q)\leftarrow(x,y)$,
    \item ${\bf D}_b$ is in the basis $(x,y)\leftarrow(x,y)$,
    \item ${\bf B}_\text{cal}$ is in the basis $(x,y)\leftarrow(p,q)$.
\end{enumerate}
if true, this would put the resulting ${\bf D}$ matrix in the basis:
\begin{equation}
    {\bf D} = \begin{bmatrix}
        D_{pp} & D_{pq} \\
        D_{qp} & D_{qq}
...
According to the documentation, this gives the ${\bf B}$ matrix in the $(q,p)\leftarrow(\theta,\phi)$ basis.
...
As argued above, the $\pamat$ matrix embodies the transformation $(\theta,\phi)\leftarrow(y,x)$.
Taken together, 

\subsection{Hyperdrive}

Hyperdrive outputs the calibration solutions in the ``Offringa'' format, described in Appendix \ref{app:offringa}.
The calibration solutions themselves are in the basis:
\begin{equation}
    {\bf D} = \begin{bmatrix}
        D_{qq} & D_{qp} \\
        D_{pq} & D_{pp}
